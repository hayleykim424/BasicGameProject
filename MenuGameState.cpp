...
MenuGameState::MenuGameState()
{
	...
	//TEXT: instruction*****************************************************************
	TTF_Font* fontInstr = TTF_OpenFont("assets/BebasNeueBold.ttf", 22);
	SDL_Color textcolourInstr = { 0, 0, 0, 255 };
	SDL_Surface* textSurfaceInstr = TTF_RenderText_Blended(fontInstr, "fruit: 100pt    angry face: -100pt & -1 life pt    2000pt -> Lvl 2", textcolourInstr);
	textTextureInstr = SDL_CreateTextureFromSurface(Globals::renderer, textSurfaceInstr);
	SDL_FreeSurface(textSurfaceInstr);



	textDestinationInstr;
	textDestinationInstr.x = 10;
	textDestinationInstr.y = 420;
	SDL_QueryTexture(textTextureInstr, NULL, NULL, &textDestinationInstr.w, &textDestinationInstr.h);
...
MenuGameState::~MenuGameState()
{
	...
	SDL_DestroyTexture(textTextureInstr);
}
...
void MenuGameState::render()
{
	...
	SDL_RenderCopy(Globals::renderer, textTextureInstr, NULL, &textDestinationInstr);
